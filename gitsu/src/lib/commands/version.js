...
function getNewVersion (currentVersion, versionArg) {
    ...
    if (!newVersion) {
        throw createError('Invalid version argument: `' + versionArg + '`. Usage: `gitsu version [<newversion> | major | minor | patch]`', 'EINVALIDVERSION');
    }
...
function saveJson (json, rPath) {
    ...
    file = path.join(rPath, 'gitsu.json');
...
function searchFolders (dir, options) {
    var pattern = "*/**/+(" + (options.skipJson ? ".git" : ".git|gitsu.json") + ")";
