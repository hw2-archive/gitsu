...
Project.prototype.install = function (decEndpoints, options, config) {
    ...
    return this._analyse()
            .spread(function (json, tree) {
                // It shows an error when issuing `gitsu install`
                // and no gitsu.json is present in current directory
                if (!that._jsonFile && decEndpoints.length === 0) {
                    throw createError('No gitsu.json present', 'ENOENT');
                }
...
Project.prototype.saveJson = function (forceCreate) {
    ...
    if (!this._jsonFile && !forceCreate) {
        this._logger.warn('no-json', 'No gitsu.json file to save to, use gitsu init to create one');
        return Q.resolve();
    }

    file = this._jsonFile || path.join(this._config.cwd, 'gitsu.json');
...
Project.prototype._readJson = function () {
    ...
    return this._json = readJson(this._config.cwd, {
        ...
        if (!assumed) {
            that._jsonFile = path.join(that._config.cwd, deprecated ? deprecated : 'gitsu.json');
        }
...
Project.prototype._readInstalled = function () {
    ...
    return this._installed = Q.nfcall(glob, '*/**/.gitsu.json', {
        cwd: componentsDir,
        dot: true
...
Project.prototype._restoreNode = function (node, flattened, jsonKey, processed) {
    ...
    mout.object.forOwn(deps, function (value, key) {
        ...
        this._restoreNode(restored, flattened, 'devDependencies', processed);
        ...
        if (local && restored !== local) {
            ...
            this._restoreNode(local, flattened, 'devDependencies', processed);
        }
    }, this);
};
