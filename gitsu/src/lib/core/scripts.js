...
var orderByDependencies = function (packages, installed, json) {
    ...
    var depsFromgitsuJson = json && json.dependencies ? mout.object.keys(json.dependencies) : [];
    var packageNames = mout.object.keys(packages);

    //get the list of the packages that are specified in gitsu.json in that order
    //its nice to maintain that order for users
    var desiredOrder = mout.array.intersection(depsFromgitsuJson, packageNames);
    //then add to the end any remaining packages that werent in gitsu.json
...
var run = function (cmdString, action, logger, config) {
    ...
    //pass env + gitsu_PID so callees can identify a preinstall+postinstall from the same gitsu instance
    var env = mout.object.mixIn({'gitsu_PID': process.pid}, process.env);
...
var hook = function (action, ordered, config, logger, packages, installed, json) {
    ...
    orderedPackages.forEach(function (pkg) {
        ...
        var script = path.join(curDir, "_gitsu/installer_hook.js");
...
var postresolved = function (config, pkgName, pkgPath, pkgMeta) {
    var script = path.join(pkgPath, "_gitsu/installer_hook.js");
