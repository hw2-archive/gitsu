...
var gitsuJson = require('upt-json');
var Q = require('q');
var fs = require("graceful-fs")

// The valid options are the same as gitsu-json#readFile.
...
function readJson (file, options) {
    ...
    function readCustom (meta) {
        ...
        var customFile = path.join(options.config.cwd, options.config.directory, depName, "gitsu.custom.json");
        try {
    ...
    // Read
    return Q.nfcall(gitsuJson.read, file, options)
            .spread(function (json, jsonFile) {
                ...
                if (err.code === 'ENOENT' && options.assume) {
                    ...
                    var meta = readCustom(gitsuJson.parse(options.assume, options));
